...
public class JiraTimesheetSynchronizer implements IJiraTimesheetSynchronizer {
    ...
    public void syncJiraTimesheetWithJiraIssues(List<IssueDTO> issues, Order order) throws ConnectorException {
        synchronizationInfo = new SynchronizationInfo(helperi18n("Synchronization"));
        ...
        if (workers == null && workers.isEmpty()) {
            synchronizationInfo.addFailedReason(helperi18n("No workers found"));
            return;
        }
        ...
        if (orderSyncInfo == null) {
            synchronizationInfo.addFailedReason(
                    helperi18n("Order \"{0}\" not found. Order probalbly not synchronized", order.getName()));
            return;
        }
        if (StringUtils.isBlank(orderSyncInfo.getKey())) {
            synchronizationInfo.addFailedReason(helperi18n("Key for Order \"{0}\" is empty", order.getName()));
            return;
        }
        ...
        for (IssueDTO issue : issues) {
            ...
            if (workLog == null) {
                synchronizationInfo.addFailedReason(helperi18n("No worklogs found for \"{0}\" key", issue.getKey()));
            } else {
                List<WorkLogItemDTO> workLogItems = workLog.getWorklogs();
                if (workLogItems == null || workLogItems.isEmpty()) {
                    synchronizationInfo.addFailedReason(helperi18n("No worklog items found for \"{0}\" issue", issue.getKey()));
                } else {
                    ...
                    if (orderElement == null) {
                        synchronizationInfo.addFailedReason(helperi18n("Order element \"{0}\" not found", code));
                    } else {
    ...
    private TypeOfWorkHours getTypeOfWorkHours() throws ConnectorException {
        ...
        if (StringUtils.isBlank(name)) {
            throw new ConnectorException(helperi18n("Hours type should not be empty to synchronine timesheets"));
        }
    ...
    private Worker getWorker(String nif) {
        ...
        synchronizationInfo.addFailedReason(helperi18n("Worker \"{0}\" not found", nif));
        return null;
    }
