...
public class ProjectImportController extends GenericForwardComposer {
    ...
    private void setBreadCrumbs(Component comp) {
        ...
        breadCrumbs.appendChild(new Image(BREADCRUMBS_SEPARATOR));
        breadCrumbs.appendChild(new Label(helperi18n("Import project")));
    }
    /**
        ...
        if (checkFileFormat(file)) {
            ...
            if (importCalendars.isChecked()) {
                ...
                try {
                    ...
                    messages.showMessage(Level.INFO, helperi18n(file + ": Calendar import successfully!"));
                } catch (InstanceNotFoundException e) {
                    messages.showMessage(Level.ERROR, helperi18n("Instance not found."));
                ...
                messages.showMessage(Level.INFO, helperi18n(file + ": Task import successfully!"));

            } else if (importAll.isChecked()) {

                try {
                    importAll(media.getStreamData(), file);
                    messages.showMessage(Level.INFO, helperi18n(file + ": Import successfully!"));
                } catch (InstanceNotFoundException e) {
                    messages.showMessage(Level.ERROR, helperi18n("Instance not found."));
                } catch (ValidationException e) {
                    messages.showMessage(Level.ERROR, e.getMessage());
                ...
                messages.showMessage(Level.WARNING, helperi18n("Select one of the options."));
            }

        } else {
            messages.showMessage(Level.ERROR, helperi18n("The only current supported formats are mpp and planner."));
        }
