...
public abstract class BaseCRUDController<T extends IHumanIdentifiable> extends GenericForwardComposer<Component> {
    ...
    public void doAfterCompose(Component comp) throws Exception {
        ...
        listWindow.setTitle(helperi18n("{0} List", getPluralEntityType()));
        showListWindow();
    }
        ...
        switch (state) {
            ...
            case CREATE:
                if (StringUtils.isEmpty(humanId))
                    title = helperi18n("Create {0}", getEntityType());
                else
                    title = helperi18n("Create {0}: {1}", getEntityType(), humanId);
                break;

            case EDIT:
                title = helperi18n("Edit {0}: {1}", getEntityType(), humanId);
                break;
    ...
    private void saveCommonActions() throws ValidationException {
        ...
        save();
        messagesForUser.showMessage(Level.INFO, helperi18n("{0} \"{1}\" saved", getEntityType(), getEntityBeingEdited().getHumanId()));
    }
    ...
    public final void confirmDelete(T entity) {
        ...
        try {
            if (Messagebox.show(
                    helperi18n("Delete {0} \"{1}\". Are you sure?", getEntityType(), entity.getHumanId()),
                    helperi18n("Confirm"), Messagebox.OK | Messagebox.CANCEL, Messagebox.QUESTION) == Messagebox.OK) {

                delete(entity);

                messagesForUser.showMessage(
                        Level.INFO,
                        helperi18n("{0} \"{1}\" deleted", getEntityType(), entity.getHumanId()));
            ...
            messagesForUser.showMessage(
                    Level.ERROR,
                    helperi18n("{0} \"{1}\" could not be deleted, it was already removed", getEntityType(), entity.getHumanId()));
        }
    }
