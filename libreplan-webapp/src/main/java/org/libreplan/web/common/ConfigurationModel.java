    ...
    private void checkEntitySequences() {
        ...
        for (EntityNameEnum entityName : EntityNameEnum.values()) {
            ...
            if (sequences.isEmpty()) {
                throw new ValidationException(helperi18n("At least one {0} sequence is needed", entity));
            }

            if (!isAnyActive(sequences)) {
                throw new ValidationException(helperi18n("At least one {0} sequence must be active", entity));
            }
    ...
    public void removeEntitySequences(final List<EntitySequence> sequences) {
        ...
        for (final EntitySequence entitySequence : toRemove) {
            try {
                ...
                throw new ValidationException(helperi18n("Some sequences to be removed do not exist"));
