...
public class ReassignCommand implements IReassignCommand {
            ...
            try {
                ...
                if (notifications != null) {
                    ...
                    updater.doUpdate(and(
                            ...
                            busyEnd(),
                            tellUserOnEnd(context, () -> helperi18n("{0} reassignations finished", reassignations.size()))));
                } else {
                    updater.doUpdate(and(
                            busyEnd(),
                            tellUserOnEnd(context, () -> helperi18n("Assignments could not be completed"))));
                }
            }
        };
    }
    ...
    private IDesktopUpdate busyStart(final int total) {
        return () -> Clients.showBusy(helperi18n("Doing {0} reassignations", total));
    }

    private IDesktopUpdate showCompleted(final int number, final int total) {
        return () -> Clients.showBusy(helperi18n("Done {0} of {1}", number, total));
    }
