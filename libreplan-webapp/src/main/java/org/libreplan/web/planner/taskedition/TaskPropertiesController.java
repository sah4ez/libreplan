...
public class TaskPropertiesController extends GenericForwardComposer<Component> {
    ...
    private void setItemsStartConstraintTypesCombo(Order order) {
        ...
        for (PositionConstraintType type : PositionConstraintType.values()) {
            ...
            if (firstCondition || secondCondition || thirdCondition) {
                Comboitem comboitem = new Comboitem(helperi18n(type.getName()));
                ...
                if ( thereIsTransition(newState) ) {
                    if ( isConsolidatedTask() ) {
                        ...
                        if( newState.equals(ResourceAllocationTypeEnum.SUBCONTRACT ) && !checkCompatibleAllocation()){
                            restoreOldState();
                            Messagebox.show(helperi18n("This resource allocation type is incompatible. The task has " +
    ...
    enum ResourceAllocationTypeEnum {
        NON_LIMITING_RESOURCES(helperi18n("Normal resource assignment")),
        LIMITING_RESOURCES(helperi18n("Queue-based resource assignation")),
        SUBCONTRACT(helperi18n("Subcontract"));
        ...
        public String toString() {
            return I18nHelper.helperi18n(option);
        }
    ...
    private void fromNonLimitingResource(ResourceAllocationTypeEnum newState) {
        ...
        if ( task.hasResourceAllocations() ) {
            if ( Messagebox.show(
                    helperi18n("Assigned resources for this task will be deleted. Are you sure?"),
                    helperi18n(WARNING), Messagebox.OK | Messagebox.CANCEL, Messagebox.QUESTION) == Messagebox.OK) {
    ...
    private void fromSubcontract(ResourceAllocationTypeEnum newState) {
        ...
        if ( task.isSubcontracted() ) {
            ...
            if ( communicationDate != null ) {
                ...
                if ( Messagebox.show(
                        helperi18n("IMPORTANT: Don't forget to communicate to subcontractor that his contract has been cancelled"),
                        helperi18n(WARNING), Messagebox.OK, Messagebox.EXCLAMATION) == Messagebox.OK ) {
    ...
    private void setEmailNotificationEntity(EmailTemplateEnum enumeration, Resource resource) {
        try {
            ...
            Messagebox.show(
                    helperi18n("You cannot email user twice with the same info"), helperi18n("Error"),
