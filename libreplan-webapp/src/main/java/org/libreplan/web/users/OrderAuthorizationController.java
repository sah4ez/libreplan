...
public class OrderAuthorizationController extends GenericForwardComposer{
        ...
        if(comboItem != null) {
            if(!readAuthorization && !writeAuthorization) {
                messagesForUser.showMessage(Level.WARNING,
                        helperi18n("No authorizations were added because you did not select any."));
                return;
            }
            ...
            if (comboItem.getValue() instanceof User) {
                ...
                if(result != null && result.size()==authorizations.size()) {
                    messagesForUser.showMessage(Level.WARNING,
                            helperi18n("Could not add those authorizations to user {0} " +
                ...
                if(result != null && result.size()==authorizations.size()) {
                    messagesForUser.showMessage(Level.WARNING,
                            helperi18n("Could not add those authorizations to profile {0} " +
                                            "because they were already present.",
    ...
    public RowRenderer getOrderAuthorizationRenderer() {
        return (row, data, i) -> {
            ...
            row.appendChild(new Label(helperi18n(profileOrderAuthorization.getAuthorizationType().getDisplayName())));
