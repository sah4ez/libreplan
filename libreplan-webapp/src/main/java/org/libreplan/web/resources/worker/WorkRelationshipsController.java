...
public class WorkRelationshipsController extends GenericForwardComposer {
    ...
    public void saveCriterionSatisfaction() {
        ...
        switch (addSatisfaction) {
            case OK:
                messagesForUser.showMessage(Level.INFO, helperi18n("Time period saved"));
                this.workerCRUDController.goToEditForm();
                break;
            case SATISFACTION_WRONG:
                messagesForUser
                        .showMessage(Level.WARNING,
                                helperi18n("Time period contains non valid data. Ending data must be older than starting date"));
                break;
            case DONT_COMPLY_OVERLAPPING_RESTRICTIONS:
                messagesForUser
                        .showMessage(Level.WARNING,
                                helperi18n("Could not save time period. Time period overlaps with another non-compatible time period"));
                this.workerCRUDController.goToEditForm();
                break;
            default:
                throw new RuntimeException(helperi18n("Unexpected: {0}", addSatisfaction));
        }
    }
