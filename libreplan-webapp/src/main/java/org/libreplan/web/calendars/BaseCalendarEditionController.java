...
public abstract class BaseCalendarEditionController extends GenericForwardComposer {
    ...
    private Checkbox createUnlimitedCheckbox() {
        ...
        unlimited.setLabel(helperi18n("Unlimited"));
        unlimited.setTooltiptext(helperi18n("Infinitely Over Assignable"));
    ...
    public String getCalendarType() {
        if ( baseCalendarModel.isDerived() ) {
            ...
            return helperi18n("Derived of calendar {0}",  getNameParentCalendar()) + currentStartDate + currentExpiringDate;
        }

        return helperi18n("Root calendar");
    }
    ...
    private String getCurrentExpiringDateLabel() {
        ...
        if ( date != null ) {
            label = " " + helperi18n("to {0}", Util.formatDate(date));
        }
    ...
    private String getCurrentStartDateLabel() {
        ...
        if ( date != null ) {
            label = " " + helperi18n("from {0}", Util.formatDate(date));
        }
    ...
    public String getTypeOfDay() {
        ...
        if ( exceptionDay != null ) {
            if ( calendar.getOwnExceptionDay(date) != null ) {
                return helperi18n("Exception: {0}", exceptionDay.getType().getName());
            } else {
                return helperi18n("Exception: {0} (Inh)", exceptionDay.getType().getName());
            }
        }

        if ( calendar.getCapacityOn(PartialDay.wholeDay(date)).isZero() ) {
            return helperi18n("Not workable day");
        }

        return helperi18n("Normal");
    }
    ...
    public void createException() {
        ...
        if ( type == null ) {
            throw new WrongValueException(exceptionTypes, helperi18n("Please, select type of exception"));
        } else {
        ...
        if ( startDate == null ) {
            throw new WrongValueException(dateboxStartDate, helperi18n("You should select a start date for the exception"));
        } else {
        ...
        if ( endDate == null ) {
            throw new WrongValueException(dateboxEndDate, helperi18n("Please, select an End Date for the Exception"));
        } else {
        ...
        if ( new LocalDate(startDate).compareTo(new LocalDate(endDate)) > 0 ) {
            throw new WrongValueException(dateboxEndDate,
                    helperi18n("Exception end date should be greater or equals than start date"));
        } else {
            Clients.clearWrongValue(dateboxEndDate);
        }
    ...
    public void acceptCreateNewVersion() {
        ...
        if ( isDerived() ) {
            ...
            if ( parentCalendars.getSelectedItem() == null ) {
                throw new WrongValueException(parentCalendars, helperi18n("cannot be empty"));
            }
        ...
        try {
            ...
            if ( e.getMessage().contains("Wrong expiring date") ) {
                throw new WrongValueException(compExpiringDate, helperi18n(e.getMessage()));
            } else {
                throw new WrongValueException(compStartDate, helperi18n(e.getMessage()));
            }
        }
    ...
    private void appendStandardEffortListcell(Listitem item, Capacity capacity) {
        Listcell listcell = new Listcell();
        listcell.appendChild(new Label(helperi18n(capacity.getStandardEffortString())));
        item.appendChild(listcell);
    }

    private void appendExtraEffortListcell(Listitem item, Capacity capacity) {
        Listcell listcell = new Listcell();
        listcell.appendChild(new Label(helperi18n(capacity.getExtraEffortString())));
        item.appendChild(listcell);
    }
    ...
    private void appendCodeListcell(final Listitem item, final CalendarException calendarException) {
        ...
        code.setConstraint("no empty:" + helperi18n("cannot be empty"));
    ...
    private void appendOriginListcell(Listitem item, CalendarException calendarException) {
        Listcell listcell = new Listcell();
        String origin = helperi18n("Inherited");

        if ( baseCalendarModel.isOwnException(calendarException) )
            origin = helperi18n("Direct");
    ...
    private Button createRemoveButton(final CalendarException calendarException) {
        ...
        if ( !baseCalendarModel.isOwnException(calendarException) ) {
            result.setDisabled(true);
            result.setTooltiptext(helperi18n("inherited exception can not be removed"));
        }
        ...
        private LocalDate getAppropiateEndDate(CalendarAvailability calendarAvailability, Date endDate) {
            if ( endDate == null ) {
                if (baseCalendarModel.isLastActivationPeriod(calendarAvailability)) {
                    ...
                    throw new IllegalArgumentException(helperi18n("End date can only be deleted in the last activation"));
                }
            }
            ...
            for (int i = 0; i < listbox.getItemCount(); i++) {
                ...
                if ( item.getChildren().size() == 5)  {
                    ...
                    if ( code != null && !code.isDisabled() && code.getValue().isEmpty() ) {
                        throw new WrongValueException(code, helperi18n("It cannot be empty"));
                    }
                }
            }
        }
