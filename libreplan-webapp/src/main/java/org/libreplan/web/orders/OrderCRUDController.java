...
public class OrderCRUDController extends GenericForwardComposer {
    ...
    private void setupGlobalButtons() {
        ...
        cancelEditionButton.addEventListener(Events.ON_CLICK, event -> Messagebox.show(
                helperi18n("Unsaved changes will be lost. Are you sure?"), helperi18n("Confirm exit dialog"),
        ...
        private void fillSchedulingModes() {
            ...
            if ( options != null && options.isEmpty() ) {
                ...
                schedulingMode.appendChild(createCombo(
                        SchedulingMode.FORWARD, helperi18n("Forward"), helperi18n("Schedule from start to deadline")));

                schedulingMode.appendChild(createCombo(
                        SchedulingMode.BACKWARDS, helperi18n("Backwards"), helperi18n("Schedule from deadline to start")));
            }
        }
            ...
            initDate.setConstraint((comp, value) -> {
                if (value == null) {
                    ...
                    if (mode == SchedulingMode.FORWARD) {
                        throw new WrongValueException(comp, helperi18n("Starting date cannot be empty in forward mode"));
                    }

                    if ( orderModel.isAnyTaskWithConstraint(PositionConstraintType.AS_SOON_AS_POSSIBLE) ) {
                        throw new WrongValueException(
                                comp,
                                helperi18n("Starting date cannot be empty because there is a task with constraint " +
                                        "\"as soon as possible\""));
                    }
                }
            });
            ...
            deadline.setConstraint((comp, value) -> {
                if (value == null) {
                    if (mode == SchedulingMode.BACKWARDS) {
                        throw new WrongValueException(comp, helperi18n("Deadline cannot be empty in backwards mode"));
                    }

                    if (orderModel.isAnyTaskWithConstraint(PositionConstraintType.AS_LATE_AS_POSSIBLE)) {
                        throw new WrongValueException(
                                comp,
                                helperi18n("Deadline cannot be empty because there is a task with constraint " +
                                        "\"as late as possible\""));
                    }
                }
            });
    ...
    protected void saveAndContinue(boolean showSaveMessage) {
        ...
        if ( orderModel.userCanRead(order, SecurityUtils.getSessionUserLoginName()) ) {
            ...
            Messagebox.show(
                    helperi18n("You don't have read access to this project"), helperi18n(INFORMATION),
    ...
    public void confirmRemove(Order order) {
        if ( orderModel.userCanWrite(order) ) {
            ...
            int status = Messagebox.show(
                    helperi18n("Confirm deleting {0}. Are you sure?", order.getName()), DELETE,
                    Messagebox.OK | Messagebox.CANCEL, Messagebox.QUESTION);
            ...
            Messagebox.show(
                    helperi18n("Not enough permissions to edit this project"), helperi18n(INFORMATION),
                    Messagebox.OK, Messagebox.INFORMATION);
        }
    }
    ...
    private void remove(Order order) {
        ...
        if ( alreadyInUse ) {
            ...
            messagesForUser.showMessage(
                    Level.ERROR,
                    helperi18n("You can not remove the project \"{0}\" because it has time tracked at some of its tasks",
                            order.getName()));
        } else {
            if ( !StringUtils.isBlank(order.getExternalCode()) ) {

                if ( Messagebox.show(
                        helperi18n("This project is a subcontracted project. If you delete it, " +
            ...
            messagesForUser.showMessage(Level.INFO, helperi18n("Removed {0}", order.getName()));
        }
    }
    ...
    public void schedule(Order order) {
        ...
        if ( orderModel.userCanRead(order, SecurityUtils.getSessionUserLoginName()) ) {
            ...
            if ( order.isScheduled() ) {
                ...
                Messagebox.show(
                        helperi18n("The project has no scheduled elements"), helperi18n(INFORMATION),
                        Messagebox.OK, Messagebox.INFORMATION);
            }
        } else {
            Messagebox.show(
    ...
    public void checkUserCanRead(Order order) {
        if ( !orderModel.userCanRead(order, SecurityUtils.getSessionUserLoginName()) ) {
            Messagebox.show(
                    helperi18n("Sorry, you do not have permissions to access this project"), helperi18n(INFORMATION),
                    Messagebox.OK, Messagebox.INFORMATION);
        }
    }
    ...
    public class OrdersRowRenderer implements RowRenderer {
        ...
        public void render(Row row, Object o, int i) throws Exception {
            ...
            appendObject(row, helperi18n(order.getState().toString()));
        ...
        private void appendButtonEdit(final Hbox hbox, final Order order) {
            ...
            buttonEdit.setTooltiptext(helperi18n("Edit"));
        ...
        private void appendButtonDelete(final Hbox hbox, final Order order) {
            if ( orderModel.userCanWrite(order) ) {
                ...
                buttonDelete.setTooltiptext(helperi18n(DELETE));
        ...
        private void appendButtonPlan(final Hbox hbox, final Order order) {
            ...
            buttonPlan.setTooltiptext(helperi18n("See scheduling"));
        ...
        private void appendButtonDerived(final Hbox hbox, final Order order) {
            ...
            buttonDerived.setTooltiptext(helperi18n("Create Template"));
            ...
            if ( !SecurityUtils.isSuperuserOrUserInRoles(UserRole.ROLE_TEMPLATES) ) {
                ...
                buttonDerived.setTooltiptext(helperi18n("Not enough permissions to create templates"));
            }
        ...
        return (comp, value) -> {
            ...
            try {
                ...
                if ( !found.getId().equals(getOrder().getId()) ) {
                    throw new WrongValueException(comp, helperi18n("project name already being used"));
                }
    ...
    public Constraint checkValidProjectCode() {
        return (comp, value) -> {
            ...
            try {
                ...
                if ( !found.getId().equals(getOrder().getId()) ) {
                    throw new WrongValueException(comp, helperi18n("project code already being used"));
                }
            } catch (InstanceNotFoundException ignored) {}
        };
    }
    ...
    public String getProjectType() {
        return isSubcontractedProject() ? helperi18n("Subcontracted by client") : helperi18n("Regular project");
    }
    ...
    public void addAskedEndDate(Datebox newEndDate) {
        ...
        if ( thereIsSomeCommunicationDateEmpty() ) {
            messagesForUser.showMessage(
                    Level.ERROR,
                    helperi18n("It will only be possible to add an end date if all the exiting ones in the table " +
                            "have already been sent to the customer."));
            return;
        }

        if ( orderModel.alreadyExistsRepeatedEndDate(newEndDate.getValue()) ) {
            messagesForUser.showMessage(Level.ERROR, helperi18n("It already exists a end date with the same date. "));
