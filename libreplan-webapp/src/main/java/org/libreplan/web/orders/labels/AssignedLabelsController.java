...
public abstract class AssignedLabelsController<T, M> extends GenericForwardComposer {
    ...
    public void onAssignLabel() {
        ...
        if (isAssigned(label)) {
            throw new WrongValueException(bdLabels, helperi18n("already assigned"));
        }
        try {
            assignLabel(label);
    ...
    public void onCreateAndAssign() {
        ...
        if (!SecurityUtils.isSuperuserOrUserInRoles(UserRole.ROLE_LABELS)) {
            throw new WrongValueException(buttonCreateAndAssign,
                    helperi18n("you do not have permissions to create new labels"));
        }

        // Check LabelType is not null
        final Comboitem comboitem = cbLabelType.getSelectedItem();
        if (comboitem == null || comboitem.getValue() == null) {
            throw new WrongValueException(cbLabelType, helperi18n("please, select an item"));
        }

        // Check Label is not null or empty
        final String labelName = txtLabelName.getValue();
        if (labelName == null || labelName.isEmpty()) {
            throw new WrongValueException(txtLabelName, helperi18n("cannot be empty"));
        }
        ...
        if (label == null) {
            ...
            if (isAssigned(label)) {
                throw new WrongValueException(txtLabelName, helperi18n("already assigned"));
            }
        }
        try {
            assignLabel(label);
