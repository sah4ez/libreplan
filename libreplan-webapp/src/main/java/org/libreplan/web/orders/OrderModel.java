...
public class OrderModel extends IntegrationEntityModel implements IOrderModel {
    ...
    public String gettooltipText(Order order) {
        ...
        result.append(helperi18n("Progress") + ": ").append(getEstimatedAdvance(order)).append("% , ");
        result.append(helperi18n("Hours invested")).append(": ").append(getHoursAdvancePercentage(order)).append("%\n");

        if (!getDescription(order).equals("")) {
            result.append(" , " + helperi18n("Description") + ": " + getDescription(order) + "\n");
        }

        String labels = buildLabelsText(order);
        if (!labels.equals("")) {
            result.append(" , " + helperi18n("Labels") + ": " + labels);
        }
