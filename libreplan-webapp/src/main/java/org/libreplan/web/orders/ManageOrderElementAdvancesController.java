...
public class ManageOrderElementAdvancesController extends GenericForwardComposer {
    ...
    private void validate() {
        if ( !validateDataForm() )
            throw new IllegalSyntaxException(helperi18n("values are not valid, the values must not be null"));


        if ( !validateReportGlobalAdvance() )
            throw new IllegalSyntaxException(helperi18n("Invalid Spread values. At least one value should be true"));
    }
    ...
    public boolean save() {
        try {
            ...
            messagesForUser.showMessage(Level.ERROR, helperi18n("Cannot create another progress of the same type"));
        } catch (DuplicateValueTrueReportGlobalAdvanceException e) {
            messagesForUser.showMessage(Level.ERROR, helperi18n("Invalid Spread values. At least one value should be true"));
    ...
    private void showInfoAbout(AdvanceAssignment advance) {
        if ( manageOrderElementAdvancesModel.isSubcontractedAdvanceTypeAndSubcontractedTask(advance) )
            showErrorMessage(helperi18n("Subcontractor values are read only " +
    ...
    public void goToRemoveLineAdvanceAssignment(Listitem listItem) {
        ...
        if ( (editAdvances.getItemCount() > 1) && (advance.getReportGlobalAdvance()) )
            ...
            showErrorMessage(helperi18n("Progress Assignment cannot be deleted or changed. " +
    ...
    public void goToRemoveLineAdvanceMeasurement(Listitem listItem) {
        ...
        if ( manageOrderElementAdvancesModel.canRemoveOrChange(advance) ) {
            ...
            showErrorMessage(helperi18n("Progress Measurement cannot be deleted. Progress Measurement already consolidated"));
    }
    ...
    public class AdvanceTypeListRenderer implements ListitemRenderer {
            ...
            maxValue.addEventListener(Events.ON_CHANGE, (EventListener) event -> {
                if (manageOrderElementAdvancesModel.hasConsolidatedAdvances(advanceAssignment))
                    throw new WrongValueException(
                            maxValue,
                            helperi18n("Progress Assignment cannot be deleted or changed. " +
                                    "Progress Assignment contains Progress Consolidations values"));
                else {
                    setPercentage();
                    reloadAdvances();
        ...
        private void appendAddMeasurement(final Hbox hbox, final Listitem listItem, Boolean readOnly) {
            ...
            if ( (advance.getAdvanceType() != null) && (advance.getAdvanceType().isQualityForm()) ) {
                ...
                addMeasurementButton.setTooltiptext(helperi18n("Progress that are reported by quality forms can not be modified"));

            } else if ( (advance.getAdvanceType() != null) && (advance.getAdvanceType().isReadOnly()) ) {
                addMeasurementButton.setDisabled(true);
                addMeasurementButton.setTooltiptext(helperi18n("This progress type cannot be modified"));

            } else if ( advance instanceof IndirectAdvanceAssignment ) {
                addMeasurementButton.setDisabled(true);
                addMeasurementButton.setTooltiptext(helperi18n("Calculated progress can not be modified"));

            } else if ( readOnly ) {
                addMeasurementButton.setDisabled(true);
                addMeasurementButton.setTooltiptext(helperi18n("Subcontractor values are read only " +
                        "because they were reported by the subcontractor company."));
            }
        ...
        private void appendRemoveButton(final Hbox hbox, final Listitem listItem, Boolean readOnly) {
            ...
            if ( (advance.getAdvanceType() != null) && (advance.getAdvanceType().isQualityForm()) ) {
                ...
                removeButton.setTooltiptext(helperi18n("Progress that are reported by quality forms cannot be modified"));

            } else if ( (advance.getAdvanceType() != null) && (advance.getAdvanceType().isReadOnly()) ) {
                removeButton.setDisabled(true);
                removeButton.setTooltiptext(helperi18n("This progress type cannot be modified"));

            } else if ( advance instanceof IndirectAdvanceAssignment ) {
                removeButton.setDisabled(true);
                removeButton.setTooltiptext(helperi18n("Calculated progress cannot be removed"));

            } else if ( manageOrderElementAdvancesModel.hasConsolidatedAdvances(advance) ) {
                removeButton.setDisabled(true);
                removeButton.setTooltiptext(helperi18n("Consolidated progress cannot be removed"));

            } else if ( readOnly ) {
                removeButton.setDisabled(true);
                removeButton.setTooltiptext(helperi18n("Subcontractor values are read only " +
                        "because they were reported by the subcontractor company"));

            } else if ( manageOrderElementAdvancesModel.hasReportedProgress(advance) ) {
                removeButton.setDisabled(true);
                removeButton.setTooltiptext(helperi18n("Advance assignment cannot be removed as " +
        ...
        private void setReportGlobalAdvance(final Listitem item) {
            ...
            if ( manageOrderElementAdvancesModel.hasAnyConsolidatedAdvanceCurrentOrderElement() ) {
                showErrorMessage(helperi18n("Spread progress cannot be changed " +
    ...
    private Constraint checkMaxValue() {
        return (comp, value) -> {
            ...
            if ( !manageOrderElementAdvancesModel.hasConsolidatedAdvances(advance) &&
                ...
                throw new WrongValueException(comp, helperi18n("The max value must be greater than 0"));
            }
        };
    }
        ...
        for (int i = 0; i < editAdvances.getChildren().size(); i++)
            ...
            if ( editAdvances.getChildren().get(i) instanceof Listitem ) {
                ...
                if ( advance != null ) {
                    ...
                    if (advance.getAdvanceType() == null )
                        throw new WrongValueException(getComboboxTypeBy(listItem), helperi18n("cannot be empty"));
                    ...
                    if ( directAdvanceAssignment != null && directAdvanceAssignment.getMaxValue() == null )
                        throw new WrongValueException(getDecimalboxMaxValueBy(listItem), helperi18n("cannot be empty"));
                }
            }
    ...
    private class AdvanceMeasurementRenderer implements ListitemRenderer {
        ...
        private void appendDecimalBoxValue(final Listitem listitem) {
            ...
            decimalbox.addEventListener(Events.ON_CHANGE, (EventListener) event -> {
                if ( manageOrderElementAdvancesModel.canRemoveOrChange(advanceMeasurement) ) {
                    ...
                    throw new WrongValueException(decimalbox, helperi18n("Progress Measurement cannot be deleted." +
        ...
        private void appendDateboxDate(final Listitem listitem) {
            ...
            date.addEventListener(Events.ON_CHANGE,(EventListener) event -> {
                if ( manageOrderElementAdvancesModel.canRemoveOrChange(advanceMeasurement) ) {
                    ...
                    throw new WrongValueException(date, helperi18n("Progress Measurement cannot be deleted." +
                            " Progress Measurement already consolidated"));
            });
        ...
        private void appendRemoveButton(final Listitem listItem) {
            ...
            if ( (advance.getAdvanceType() != null) && (advance.getAdvanceType().isQualityForm()) ) {
                ...
                removeButton.setTooltiptext(helperi18n("Progress measurements that are reported " +
                        "by quality forms cannot be removed"));

            } else if ( (advance.getAdvanceType() != null) && (advance.getAdvanceType().isReadOnly()) ) {
                removeButton.setDisabled(true);
                removeButton.setTooltiptext(helperi18n("This progress type cannot cannot be removed"));

            } else if ( advance.isFake() ) {
                removeButton.setDisabled(true);
                removeButton.setTooltiptext(helperi18n("Calculated progress measurements cannot be removed") );

            } else if ( manageOrderElementAdvancesModel.hasConsolidatedAdvances(measure) ) {
                removeButton.setDisabled(true);
                removeButton.setTooltiptext(helperi18n("Consolidated progress measurement cannot be removed"));

            } else if ( manageOrderElementAdvancesModel.isAlreadyReportedProgress(measure) ) {
                removeButton.setDisabled(true);
                removeButton.setTooltiptext(helperi18n("Values already sent to the customer. Values cannot be changed "));
        ...
        private void showMessagesConsolidation(LocalDate date) {
            String message = helperi18n("Progress measurement cannot be canged to {0}, because it is consolidated", date);
    ...
    private Button createAddMeasurementButton() {
        ...
        addButton.setLabel(helperi18n("Add measure"));
        addButton.setClass("add-button");
        addButton.setTooltiptext(helperi18n("Add new progress measurement"));
    ...
    private void showMessageNotAddMoreAdvances() {
        String message = helperi18n("All progress types have already been assigned.");
        increaseScreenHeight();
    ...
    private void showMessageDeleteSpread() {
        String message = helperi18n("Spread progress cannot be removed. Please select another progress as spread.");
        showErrorMessage(message);
    }
    ...
    private String validateValueAdvanceMeasurement(AdvanceMeasurement measurement) {
        if ( manageOrderElementAdvancesModel.greatThanMaxValue(measurement) )
            return helperi18n("Value is not valid. It must be smaller than max value");

        if ( !manageOrderElementAdvancesModel.isPrecisionValid(measurement) )
            return helperi18n("Value must be a multiple of the precision value of the progress type: {0}",
        ...
        if ( manageOrderElementAdvancesModel.lessThanPreviousMeasurements() )
            return helperi18n("Invalid value. Value must be greater than the value of previous progress.");
    ...
    private String validateDateAdvanceMeasurement(LocalDate value, AdvanceMeasurement measurement) {
        ...
        if ( !manageOrderElementAdvancesModel.isDistinctValidDate(value, measurement))
            return helperi18n("Invalid date. Date must be unique for this Progress Assignment");
        ...
        if ( !isReadOnlyAdvanceMeasurements() ) {
            ...
            if ( consolidatedUntil != null  && consolidatedUntil.compareTo(measurement.getDate()) >= 0 ) {
                return helperi18n("Date is not valid, it must be later than the last progress consolidation");
            }
            if ( manageOrderElementAdvancesModel.isAlreadyReportedProgressWith(value) ) {
                return helperi18n("Date is not valid, it must be later than the last progress reported to the customer");
            }
        }
