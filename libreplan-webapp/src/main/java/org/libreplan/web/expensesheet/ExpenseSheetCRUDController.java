...
public class ExpenseSheetCRUDController
    ...
    private boolean validateNewLine() {
        if (expenseSheetModel.getNewExpenseSheetLine().getDate() == null) {
            throw new WrongValueException(this.dateboxExpenseDate, helperi18n(NOT_EMPTY));
        }

        if (expenseSheetModel.getNewExpenseSheetLine().getOrderElement() == null) {
            throw new WrongValueException(this.bandboxTasks, helperi18n(NOT_EMPTY));
        }

        BigDecimal value = expenseSheetModel.getNewExpenseSheetLine().getValue();

        if (value == null || value.compareTo(BigDecimal.ZERO) < 0) {
            throw new WrongValueException(this.dboxValue, helperi18n("cannot be empty or less than zero"));
        }
        return true;
    }
    ...
    public void confirmRemove(ExpenseSheetLine expenseSheetLine) {
        int status = Messagebox.show(
                helperi18n("Confirm deleting {0}. Are you sure?", getExpenseSheetLineName(expenseSheetLine)),
    ...
    private String getExpenseSheetLineName(ExpenseSheetLine expenseSheetLine) {
        if (expenseSheetLine != null) {
            ...
            if (date != null && task != null) {
                return helperi18n("expense line of the ") + task.getName() + " - " + date;
            }
        }
        return helperi18n("item");
    }
    ...
    private class ExpenseSheetLineRenderer implements RowRenderer {
        ...
        private void appendDateInLines(Row row) {
            ...
            dateboxExpense.setConstraint("no empty:" + helperi18n(NOT_EMPTY));
        ...
        private void appendOrderElementInLines(Row row) {
            ...
            bandboxSearch.setBandboxConstraint("no empty:" + helperi18n(NOT_EMPTY));
            row.appendChild(bandboxSearch);
        }
            ...
            return (comp, value) -> {
                if (!getExpenseSheet().isCodeAutogenerated()) {
                    ...
                    if (code == null || code.isEmpty()) {
                        throw new WrongValueException(comp, helperi18n("Code cannot be empty."));
                    } else {
                        ...
                        if (!getExpenseSheet().isNonRepeatedExpenseSheetLinesCodesConstraint()) {
                            ...
                            throw new WrongValueException(comp, helperi18n("The code must be unique."));
                        }
                    }
                }
            };
    ...
    public Constraint checkConstraintExpenseValue() {
        return (comp, value) -> {
            ...
            if (expenseValue == null || expenseValue.compareTo(BigDecimal.ZERO) < 0) {
                throw new WrongValueException(comp, helperi18n("must be greater or equal than 0"));
            }
        };
    }
    ...
    public Constraint checkConstraintExpendeCode() {
        return (comp, value) -> {
            if (!getExpenseSheet().isCodeAutogenerated()) {
                ...
                if (code == null || code.isEmpty()) {
                    throw new WrongValueException(comp, helperi18n("The code cannot be empty and it must be unique."));
                } else if (!getExpenseSheet().isUniqueCodeConstraint()) {

                    throw new WrongValueException(
                            comp, helperi18n("it already exists another expense sheet with the same code."));
                }
            }
        };
    }
    ...
    protected String getEntityType() {
        return helperi18n("Expense Sheet");
    }

    @Override
    protected String getPluralEntityType() {
        return helperi18n("Expense Sheets");
    }
    ...
    private String getType(ExpenseSheet expenseSheet) {
        return expenseSheet != null && expenseSheet.isPersonal() ? helperi18n("Personal") : helperi18n("Regular");
    }
